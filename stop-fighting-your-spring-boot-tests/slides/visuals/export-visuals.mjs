// Exports the test-choice decision scenes to PNG files in ../assets.
...
const sceneUrl = pathToFileURL(path.join(hereDir, 'test-choice.html')).href;
const deviceScaleFactor = Number(process.env.SCALE || 2);

const scenes = [
  { query: '', file: 'test-choice.png', width: 1600, height: 800 }
];
...
async function exportScene({ kind, browser }, scene) {
  ...
  const sceneHandle = await page.$('#scene');
  await sceneHandle.screenshot({ path: outputPath, omitBackground: true });
